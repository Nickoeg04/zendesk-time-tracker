...
const timer = async () => {
  ...
  const dagOverzicht = async () => {
    const data = await browser.storage.local.get("timers");
    const currentDiv = document.getElementById("timersVandaag");
    if (data.timers && data.timers[date]) {
      for (const ticket in data.timers[date]) {
        const newDiv = document.createElement("div");

        let tijd = timeToString(data.timers[date][ticket].elapsed);
        const newContent = document.createTextNode(
          ticket.toString() + "-" + tijd,
        );
